          ...
          <td colSpan={2}>
            <CenterColumn>
              <MainTitlecontainer>
                ...
                <MainPriceContainer>
                  <MainPrice>
                    <MainPriceCoin>{property?.operations[0]?.prices[0]?.currency}</MainPriceCoin>
                    <Price>{formatToMoney(property?.operations[0]?.prices[0]?.price)}</Price>
                  </MainPrice>
                  <Rooms>
                    {property.room_amount} {property.room_amount === 1 ? "Ambiente" : "Ambientes"}
                    {Math.round(property?.parking_lot_amount) > 0 && ` + ${Math.round(property?.parking_lot_amount)} Cocheras`}
                  </Rooms>                
                </MainPriceContainer>
              </MainTitlecontainer>
              <GridContainer>
                <MainImage>
                  <img src={photoGallery[0]?.src} alt="Main property image" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </MainImage>
                <SmallImages>
